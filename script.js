...
function openDetailStackPage(projectId) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${
    project.title
  }</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:#f8f9fa;color:#2c2c2c;line-height:1.6;padding:20px}.header{text-align:center;margin-bottom:30px;padding:20px;background:#e8f4f8;border-radius:12px;border:2px solid #d1e7dd}.all-content{display:flex;flex-direction:column;gap:20px;max-width:900px;margin:0 auto}.image-item img{width:100%;height:auto;display:block;object-fit:contain;background:#fff;border-radius:12px;box-shadow:0 4px 15px rgba(0,0,0,.1);border:1px solid #e8e8e8}.close-btn{position:fixed;top:14px;right:14px;background:#111;color:#fff;border:none;padding:10px 14px;border-radius:20px;cursor:pointer;font-weight:600;box-shadow:0 4px 12px rgba(0,0,0,.25);z-index:1000}.close-btn:hover{background:#333}</style></head><body><button class="close-btn" onclick="window.close()">닫기 ×</button><div class="header"><h1>${
    project.title
